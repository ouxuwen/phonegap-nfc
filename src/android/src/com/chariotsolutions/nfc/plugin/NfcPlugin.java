...
public class NfcPlugin extends CordovaPlugin implements NfcAdapter.OnNdefPushCompleteCallback {
                    ...
                    if (ndef != null) {
                        ...
                        if (ndef.isWritable()) {
                            ...
                            if (ndef.getMaxSize() < size) {
                                callbackContext.error("Tag capacity is " + ndef.getMaxSize() + " bytes, message is "
                                        + size + " bytes.");
                            } else {
        ...
        getActivity().runOnUiThread(new Runnable() {
            public void run() {
                ...
                if (nfcAdapter != null && !getActivity().isFinishing()) {
                    try {
                        nfcAdapter.enableForegroundDispatch(getActivity(), getPendingIntent(), getIntentFilters(),
                                getTechLists());
    ...
    private void fireTagEvent(Tag tag) {
        ...
        try {
            ...
            nfcA.connect();
            byte[] SELECT = { (byte) 0x30, (byte) 0x10 };
            byte[] result = nfcA.transceive(SELECT);
            int data_len = ((result[0] & 0x0f) << 8) + ((result[1] & 0xff));
            String ins = Util.ByteArrayToHexString(result);
            res = Util.hex2Decimal(ins.substring(0, 16));
            Log.i(TAG, "是否已写入数据" + result[0] + "，写入数据长度：" + data_len);
        } catch (IOException e) {
            e.printStackTrace();
            Log.e(TAG, "读卡失败");
        } catch (Exception e) {
        ...
        JSONObject obj = Util.tagToJSON(tag);
        if(res != null){
            try {
                obj.put("rid",res);
            } catch (Exception e) {
        ...
        String command = MessageFormat.format(javaScriptEventTemplate, TAG_DEFAULT,obj );
        Log.v(TAG, command);
        this.webView.sendJavascript(command);
    }
